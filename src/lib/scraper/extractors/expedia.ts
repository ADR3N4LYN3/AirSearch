...
/**
 * Expedia shares the same Expedia Group design system as Vrbo.
...
export async function extractExpedia(page: Page): Promise<ScrapedListing[]> {
  ...
  return page.evaluate(() => {
    ...
    cards.forEach((card) => {
      ...
      // Expedia uses /10 scale → convert to /5
      ...
      if (linkEl) {
        url = linkEl.href.startsWith("http") ? linkEl.href : `https://www.expedia.fr${linkEl.getAttribute("href")}`;
      }
